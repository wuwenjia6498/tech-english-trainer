...
JSON 格式要求（严格遵守，不要多余字段）：
{
  ...
  "category": "所属分类，从以下选一个：编程基础、开发环境、开发与部署、数据格式、网络与后端、前端开发、AI 与数据科学、安全与认证",
  "translation": "简洁的中文释义",
  "tech_context": "技术场景，用大白话解释，不超过2句话",
  "code_example": "代码示例，第一行必须是注释格式：# 单词 — 中文释义，第二行开始是极简代码（1~3行）"
}`;
...
export async function POST(request: Request) {
  try {
    ...
    const { phonetic, category, translation, tech_context, code_example } = result;
    ...
    return NextResponse.json({ phonetic, category: category || "", translation, tech_context, code_example });
  } catch (err: unknown) {
